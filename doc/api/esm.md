...
When using the `--type` flag with an entry point, the algorithm below behaves
as if there is a package.json file in the folder of the entry point containing
a `type` property set to the flag value (provided there isn't already a
package.json in the entry folder with a conflicting type value).
