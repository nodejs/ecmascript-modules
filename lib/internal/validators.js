...
/*
 * Copied from Acorn identifier.js
 * (https://github.com/acornjs/acorn/blob/master/acorn/src/identifier.js)
 */
...
function isValidIdentifier(id, astral = true) {
  if (typeof id !== 'string' || id.length === 0)
    return false;
  let index = 0;
  let code = id.charCodeAt(index++);
  if (code > 0xd7ff && code < 0xe000) {
    const next = id.charCodeAt(index++);
    code = (code << 10) + next - 0x35fdc00;
  }
  if (!isIdentifierStart(code, astral))
    return false;
  while (index < id.length) {
    ...
    if (!isIdentifierChar(code, astral))
      return false;
  }
  return true;
}
